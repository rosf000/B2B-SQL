...
- **第一站（核心戰略與思想底座）**：[00 轉職戰略與導讀模組](./00_轉職戰略與導讀/README.md)（B2B 優勢定位、三層完成制度與排程法）
- **實戰第一模組（起步第一天）**：[Month 01 SQL 基礎與 DBeaver 安裝](./01_Month01_SQL基礎/README.md)（環境建置、SQL 核心查詢與結業偵探考題）
- **旗艦作品預覽（目標終點線）**：[Month 08 旗艦主力專案：B2B 客戶數據系統](./08_Month08_旗艦主力專案_B2B客戶數據系統/README.md)（7 大 Portfolio Gate 通關標準）
