...
> **📌 本章定位**：正規化（Normalization）是關聯式資料庫消除資料冗餘與異常的「骨架工程」。其核心心智模型只有一句話：**「一個實體一張表，一件事只在一個地方說一次」**。
>
> **⚠️ 痛點場景**：很多工程師只會寫 `SELECT`，建表時把所有欄位塞在一張寬表（order_mega_table）。結果：
> - 客戶改電話，全表要更新 50 筆，漏改一筆就造成對帳崩潰（更新異常）
> - 想上架新商品，卻因還沒有訂單產生而無法寫入（新增異常）
> - 刪除一筆測試訂單，結果連同歷史唯一的供應商資料一起人間蒸發（刪除異常）
>
> **💡 學習策略**：先定位（識別三種異常）➔ 再理解（1NF~3NF 函數相依）➔ 再操作（動手拆表）➔ 再回收（何時該反正規化）。遇到練習題請先在紙上畫出 ER 實體關係，切勿直接看解答！
...
> 💡 **自我檢驗規範**：請拿出一張白紙或打開空白 SQL 編輯器，先獨立寫出 DDL 與關係設計，再展開解答對照！
...
- 觀察 `items` 欄位：每個格子裡塞了多個項目，且每個項目還包含了兩個不同維度的資訊（代號與數量）。
- 違反了 1NF「原子性（Atomicity）」原則。
- 解決方法：將採購單拆成「採購單主表（Header）」與「採購單明細表（Lines）」，明細表以 `(po_id, product_code)` 作為複合主鍵。
</details>
...
- 檢視主鍵是複合主鍵 `(student_id, course_id)`。
- 逐一檢查非主鍵欄位：
  - `grade`：必須知道「哪個學生」修「哪門課」才能決定成績 ➔ 完全函數相依（符合 2NF）。
  - `course_name` 與 `instructor`：只要知道 `course_id` 就能完全決定，跟 `student_id` 無關！➔ 部分函數相依（違反 2NF）。
</details>
...
- 採用 Header-Line 雙層架構：`quotations` (1) ➔ `quotation_items` (N)。
- 狀態約束：用 `CHECK (status IN (...))` 避免髒狀態。
- 成交追蹤：在 `orders` 表增加 FK `quotation_id` 指向報價單，方便轉換與反查漏單。
</details>
