...
> [!TIP]
> **求職作品集最佳選型策略**：
> 1. **展示層（Live Demo）**：優先使用 Render / Railway 部署 FastAPI + PostgreSQL。利用免費/低成本層級換取 100% 穩定的 HTTPS 網址，直接貼在履歷第一頁。
> 2. **深度技術證明（技術面試）**：在 GitHub 專案保留完整的 `docker-compose.yml` 與 Nginx 反向代理配置，向面試官展示你完全具備自建 Linux VPS、防火牆與反向代理的高級運維工程能力。
...
> [!CAUTION]
> **雲端主機裸奔警告：上線 24 小時內必遭撞庫攻擊**  
> 剛開通的雲端 VPS 若維持預設 root 密碼登入與 Port 22，公網機器人通常在數分鐘內就會發動每秒上千次的 SSH 字典攻擊。務必嚴格依循下方 SOP：**改自訂埠號、停用密碼驗證、僅限 SSH Key 登入、掛載 UFW 防火牆與 Fail2ban**！
...
│ 核心觀念          │ 工程實踐重點與面試得分點                                 │
├───────────────────┼──────────────────────────────────────────────────────────┤
│ PaaS vs VPS       │ PaaS 專注快速 Live Demo；自建 VPS 展現 Linux 運維與資安底蘊 │
│ VPS 加固四步法    │ 停用 root、改非預設 SSH 埠、強制金鑰驗證、UFW + Fail2ban │
│ Nginx 反向代理    │ 負責 SSL 終端解密、靜態資源快取與微服務統一路徑轉發      │
│ Let's Encrypt     │ 透過 Certbot 實現每 90 天自動輪換 HTTPS 憑證             │
│ 異地容災與監控    │ pg_dump 異地備份至 S3/R2，搭配 UptimeRobot + Sentry 監控  │
└───────────────────┴──────────────────────────────────────────────────────────┘
```
...
- **前一篇**：[01_Docker進階_多階段建置與容器安全.md](./01_Docker進階_多階段建置與容器安全.md)（多階段構建與容器安全加固）
- **下一篇**：[03_Production_Gate_生產部署檢核表.md](./03_Production_Gate_生產部署檢核表.md)（10 大生產檢核標準與冒煙測試）
