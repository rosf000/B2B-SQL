...
- **`HAVING` 才能過濾聚合函數**：因為 `WHERE`（第 2 步）發生在分組之前，根本還沒計算總數；而 `HAVING`（第 4 步）發生在分組之後，所以只有 `HAVING` 能過濾 `COUNT` / `SUM`！
- **`WHERE` 不能用欄位別名**：因為 `WHERE`（第 2 步）執行時，`SELECT`（第 5 步）還沒執行，你在後面取的別名，前面根本還不認識！
- **`ORDER BY` 可以用欄位別名**：因為 `ORDER BY`（第 6 步）執行時，`SELECT`（第 5 步）已經執行完畢，你在前面取的別名，此時已經可以直接拿來排序！
