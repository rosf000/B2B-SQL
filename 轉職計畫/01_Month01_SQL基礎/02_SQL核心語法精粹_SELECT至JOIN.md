...
> [!WARNING]
> ⚠️ **養成好習慣：不要寫 `SELECT *`！**  
> 初學者很容易圖省事寫 `SELECT * FROM customers`，一次撈出所有欄位。但在業界這是明確的壞習慣：
...
#### 商業需求情境
> 採購與營運主管說：「請找出所有售價在 **5 萬元以上** 的高單價產品，依售價由高到低排序，只看最貴的前 3 名；另外想知道我們目前在哪些城市擁有客戶（重複的城市不要重複列出）。」
...
-- 2. 模糊搜尋：找出名稱帶有 'Cloud' 或 'Suite' 的產品 (ILIKE 不分大小寫)
SELECT product_name, category, unit_price
FROM products
WHERE product_name ILIKE '%Cloud%' OR product_name ILIKE '%Suite%';
...
**語句 3 的去重城市結果（共 6 個不重複城市）：**  
`Hsinchu`, `Taipei`, `New Taipei`, `Taichung`, `Kaohsiung`, `Tainan`。
...
👉 **結論**：
- `WHERE` 執行在分組之前，它只能對原始單列進行篩選。
- `HAVING` 執行在分組與計算之後，因此**聚合函數（COUNT、SUM）的篩選只能寫在 HAVING！**
- `SELECT` 在 `WHERE` 之後才執行，這就是為什麼你在 `WHERE` 裡面不能使用在 `SELECT` 定義的欄位別名！
...
| :---: | :--- | :---: | :--- |
| 5 | ORD-2024-005 | 450000.00 | Tier 1 (Enterprise) |
| 1 | ORD-2024-001 | 360000.00 | Tier 1 (Enterprise) |
| 11 | ORD-2024-011 | 290000.00 | Tier 2 (Mid-Market) |
| 4 | ORD-2024-004 | 240000.00 | Tier 2 (Mid-Market) |
| 3 | ORD-2024-003 | 80000.00 | Tier 3 (SMB) |
...
> [!TIP]
> 💡 **記憶口訣**：`COALESCE` = 「遇到 NULL 就換成 ___」；`NULLIF` = 「遇到 ___ 就變成 NULL」。在 03 題庫的 Q19、Q23、Q24、Q26、Q29、Q30 中都會用到！
