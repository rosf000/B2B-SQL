# 🚀 B2B AI-Native Data Engineer — 12 個月轉職計畫

> **「你的核心競爭力不是『我會 Python』，而是『我懂 B2B 商業問題，並能用 AI + 技術解決它』。」**

---

## 🎯 職涯定位

```
B2B Domain Knowledge
        +
SQL / Database / Python / ETL
        +
AI Agents / Automation
        =
B2B Data / Automation Engineer
```

**目標職稱：** Data Engineer ／ Data Automation Engineer ／ B2B Data Platform Engineer

**核心優勢：** 非本科轉職者的最強護城河 — B2B 商業背景 × 資料工程能力 × AI 工具驅動

---

## 🗺️ 學習地圖（2026–2027 AI-Native 版）

| 階段 | 月份 | 主題 | 定位 |
|:---|:---:|:---|:---|
| **Phase 1** | M1–M3 | SQL + Database + Git | 紮實地基，從第一天就 commit |
| **Phase 2** | M4–M6 | Python + ETL + Pandas | 工具人 → 資料工程師 |
| **Phase 3** | M7 | Linux + Docker + AI Workflow | 工程素養 + AI 協作流程建立 |
| **Phase 4** | M8–M9 | 旗艦專案 + FastAPI API 化 | 面試主力核心作品 |
| **Phase 5** | M10–M11 | 部署 + Airflow/AI Agent | 生產環境思維 |
| **Phase 6** | M8+ | 持續求職（M8 起就開始投） | 不等 M12 才看市場 |

---

## 📂 教材目錄

| 模組 | 主題 | 核心內容 | 狀態 |
|:---|:---|:---|:---:|
| **00 導讀** | [轉職戰略與導讀](./轉職計畫/00_轉職戰略與導讀/README.md) | B2B 優勢定位、AI-Native 學習方法、求職節奏 | ✅ |
| **Month 01** | [SQL 基礎 + Git 起手](./轉職計畫/01_Month01_SQL基礎/README.md) | SELECT/JOIN/GROUP BY、Git init、GitHub 第一次 commit | ✅ |
| **Month 02** | [SQL 進階與分析](./轉職計畫/02_Month02_SQL進階與分析/README.md) | CTE、Window Functions、Project 1 銷售分析 | ✅ |
| **Month 03** | [資料庫設計與建模](./轉職計畫/03_Month03_資料庫設計與建模/README.md) | ERD、正規化、Index、ACID、AI 生成 Schema → 你來 Review | ✅ |
| **Month 04** | [Python 基礎（目標導向）](./轉職計畫/04_Month04_Python基礎與實用工具/README.md) | 只學解決 B2B 問題所需的 Python，不背語法全集 | ✅ |
| **Month 05** | [Python × ETL × Database](./轉職計畫/05_Month05_Python與資料庫整合自動化/README.md) | ETL Pipeline、Logging、Config、Retry、Project 2 | ✅ |
| **Month 06** | [Pandas + API 資料處理](./轉職計畫/06_Month06_Pandas與API資料處理/README.md) | 資料清洗、REST API 入庫、AI 協助 Code Review | ✅ |
| **Month 07** | [Linux + Docker + AI Workflow](./轉職計畫/07_Month07_工程素養_Git與Linux/README.md) | Linux 基礎、Docker Compose、AI Coding 工作流建立 | ✅ |
| **Month 08** | [🔥 旗艦：AI-Augmented B2B Data Platform](./轉職計畫/08_Month08_旗艦主力專案_B2B客戶數據系統/README.md) | B2B 系統 + AI Agent 整合、面試核心作品、**開始投履歷** | ✅ |
| **Month 09** | [FastAPI API 化](./轉職計畫/09_Month09_後端開發_FastAPI/README.md) | 把旗艦專案 API 化、CRUD、Swagger、基礎 Auth | ✅ |
| **Month 10** | [Docker 部署 + 雲端](./轉職計畫/10_Month10_容器化與部署_Docker/README.md) | docker-compose 一鍵啟動、雲端部署、可 Demo 的 URL | ✅ |
| **Month 11** | [Airflow / AI Agent 系統](./轉職計畫/11_Month11_AI賦能_智慧資料助理/README.md) | Data Pipeline 排程（Airflow）+ AI Agent 多角色架構 | ✅ |
| **Month 12** | [求職衝刺](./轉職計畫/12_Month12_轉職衝刺與求職寶典/README.md) | 履歷精修、面試 50 題、根據 JD 補強、持續投遞 | ✅ |

---

## 🔥 四大主力作品

```
┌─────────────────────────────────────────────────────────────────┐
│ Project 1：SQL Business Analysis (M2)                           │
│   ➜ 銷售資料分析、Cohort、RFM、CTE、Window Functions            │
├─────────────────────────────────────────────────────────────────┤
│ Project 2：Automated ETL Pipeline (M5)                          │
│   ➜ Excel → Python → 清洗驗證 → PostgreSQL，含 Logging/Config  │
├─────────────────────────────────────────────────────────────────┤
│ Project 3：AI-Augmented B2B Data Platform (M8–M10) 🏆 旗艦     │
│   ➜ B2B 資料庫設計 + ETL + FastAPI + Docker + AI Agent          │
│   ➜ 輸入：「找出三個月沒下單但過去消費超過 50 萬的客戶」         │
│   ➜ 輸出：AI 自動生成 SQL → 驗證 → 查詢 → 自然語言報告          │
├─────────────────────────────────────────────────────────────────┤
│ Project 4：Data Pipeline with Airflow (M11)                     │
│   ➜ 定時排程 ETL、資料品質監控、告警通知                        │
└─────────────────────────────────────────────────────────────────┘
```

---

## 📋 年度 Checklist（AI-Native 版）

- [ ] **M1**：Git + GitHub 第一天就建立，SQL 開始就 commit
- [ ] **M2**：Window Functions 搞懂，完成 Project 1，上傳 GitHub
- [ ] **M3**：設計 B2B ER 圖，用 AI 生成 Schema 再自己找問題
- [ ] **M4**：Python 只學 B2B 工作需要的，寫出 3 個實用小工具
- [ ] **M5**：完成 ETL Pipeline，有 Logging / Config / Error Handling
- [ ] **M6**：API 資料入庫，Pandas 清洗，AI Review 自己的 Code
- [ ] **M7**：Linux + Docker Compose，建立 AI Coding 工作流習慣
- [ ] **M8**：旗艦作品完成，架構圖、Demo、README 完整 → **開始投履歷**
- [ ] **M9**：B2B 系統 API 化，Swagger 文件完整
- [ ] **M10**：docker-compose 一鍵啟動，部署到雲端有可存取 URL
- [ ] **M11**：Airflow 排程 ETL，或 AI Agent 多角色系統擇一深入
- [ ] **M12**：根據面試回饋補強，每週追蹤投遞結果

---

## ⚡ AI-Native 學習原則

```
每個技術主題的學習循環：

自己先想 → 自己先寫 → AI 比較差異 → 理解為什麼 → 改進

不是：AI 生成 → 複製貼上 → 完成
```

**你的學習不是「背技術」，而是「能設計解決方案、能驗證 AI 產出、能對結果負責」。**
